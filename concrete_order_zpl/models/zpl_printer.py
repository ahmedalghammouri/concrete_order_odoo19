...
class ZPLPrinter(models.Model):
    _name = 'concrete.zpl.printer'
    _description = 'ZPL Thermal Printer for Concrete'
    _inherit = ['mail.thread', 'mail.activity.mixin']

    name = fields.Char(string='Printer Name', required=True, tracking=True)
    active = fields.Boolean(default=True, tracking=True)
    is_default = fields.Boolean(string='Default Printer', default=False, tracking=True)
    ...
    def _compute_print_count(self):
        for rec in self:
            rec.print_count = self.env['concrete.zpl.print.job'].search_count([('printer_id', '=', rec.id)])

    def action_test_connection(self):
        self.ensure_one()
        try:
            test_zpl = "^XA^FO50,50^ADN,36,20^FDTest Print^FS^XZ"
            self._send_to_printer(test_zpl)
...
class ZPLPrintJob(models.Model):
    _name = 'concrete.zpl.print.job'
    _description = 'ZPL Print Job'
    _order = 'create_date desc'

    name = fields.Char(string='Job Name', required=True)
    printer_id = fields.Many2one('concrete.zpl.printer', string='Printer', required=True, ondelete='cascade')
    ticket_id = fields.Many2one('concrete.delivery.ticket', string='Delivery Ticket', ondelete='set null')
